...
def generate(model, opt):
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Generating Samples'):
            ...
            np.save(f"gen_sample_{i}.npy", gen.numpy())
        ...
        samples_numpy = samples.cpu().numpy()  # 确保数据在 CPU 上并转换为 NumPy 数组
        npy_save_path = os.path.join(str(Path(opt.eval_path).parent), 'generated_point_cloud.npy')
        np.save(npy_save_path, samples_numpy)

        print(f"点云数据已保存为 {npy_save_path}")
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=128, help='input batch size')#原来是50
    ...
    parser.add_argument('--model', default='output/train_generation/2025-03-14-21-01-13/epoch_299.pth',required=True, help="path to model (to continue training)")
