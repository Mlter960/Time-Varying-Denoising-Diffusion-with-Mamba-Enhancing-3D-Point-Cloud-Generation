...
def visualize_pointcloud_batch(path, pointclouds, pred_labels, labels, categories, vis_label=False, target=None, elev=30, azim=225):
    ...
    for idx, pc in enumerate(pointclouds):
        ...
        # 添加的代码提取可视化用的点坐标 (x, z, y)
        coords = np.stack([pc[:, 0], pc[:, 2], pc[:, 1]], axis=1)  # (N, 3)

        # 构造 ShapeNetCore.v2.PC15k 类似的数据结构（分类）
        save_data = {
            'point': coords.astype(np.float32),  # 3D 点坐标
            'cls': labels[idx].item()  # 类别 index，如 0 表示 'chair'
        }

        np.save(f"{path}_pc_{idx}.npy", save_data)
