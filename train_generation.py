...
class GaussianDiffusion:
    ...
    def compute_mmd(self,x_gen, x_real, kernel='rbf', sigma=1.0):
        """x_gen, x_real: [B, N, 3]"""
        B = x_gen.size(0)
        x_gen = x_gen.reshape(B, -1)  # [B, N*3]
        x_real = x_real.reshape(B, -1)

        def gaussian_kernel(a, b):
            a = a.unsqueeze(1)  # [B, 1, D]
            b = b.unsqueeze(0)  # [1, B, D]
            return torch.exp(-torch.sum((a - b) ** 2, dim=-1) / (2 * sigma ** 2))

        K_gg = gaussian_kernel(x_gen, x_gen).mean()
        K_rr = gaussian_kernel(x_real, x_real).mean()
        K_gr = gaussian_kernel(x_gen, x_real).mean()

        mmd = K_gg + K_rr - 2 * K_gr
        return mmd

    def p_losses(self, denoise_fn, data_start, t, noise=None, lambda_cd=1.0):
        """
        ...
        if self.loss_type == 'mse':
            ...
            loss_cd = cd_loss(data_t, data_start)  # x_gen 和 x_real: [B, N, 3]
            losses += lambda_cd * loss_cd
...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        # num_classes = 3, embed_dim = 64, use_att = True
...
def chamfer_distance(pc1, pc2):
    """
    pc1, pc2: [B, 3, N]
    Return: scalar CD loss
    """
    pc1 = pc1.transpose(1, 2)  # [B, N, 3]
    pc2 = pc2.transpose(1, 2)  # [B, N, 3]
    dist = torch.cdist(pc1, pc2, p=2)  # [B, N, N]
    min_dist1 = dist.min(dim=2)[0]  # [B, N]
    min_dist2 = dist.min(dim=1)[0]  # [B, N]
...
def parse_args():
    ...
    parser.add_argument('--dataroot', default='data/ShapeNetCore.v2.PC15k/ShapeNetCore.v2.PC15k/')
