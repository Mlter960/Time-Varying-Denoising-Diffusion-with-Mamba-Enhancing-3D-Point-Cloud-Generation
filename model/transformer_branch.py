...
class MambaTblock(nn.Module):
    def __init__(self, latent_size, bimamba_type, norm_layer=nn.LayerNorm, drop_path=0.2 ):
        super().__init__()
        self.norm =norm_layer(latent_size)
        self.drop_path = DropPath(drop_path)
        self.mixer = Mamba(latent_size, bimamba_type=bimamba_type)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.mamba = nn.ModuleList([
            MambaTblock(latent_size, bimamba_type=bimamba_type) for _ in range(depth)
        ])

        self.apply(self.mamba_initialize_weights)
    ...
    def mamba_initialize_weights(self, m):
    ...
    def forward(self, x, t, coords):
        ...
        for mamba in self.mamba:
            x = mamba(x, c, pe, time, coords)                      # (N, T, D) x:(24, 256, 512)
            # x = block(x, c)        
        x = self.FinalLayer(x, c) # x:(24, 256, 256)
